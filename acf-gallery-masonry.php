<?php
/**
 * Plugin Name: ACF Gallery Masonry + Lightbox
 * Plugin URI: https://github.com/graphikup/acf-gallery-masonry
 * GitHub Plugin URI: https://github.com/graphikup/acf-gallery-masonry
 * Description: Shortcode [acf_gallery] pour afficher un champ Galerie ACF sous forme de grille Masonry responsive avec une lightbox.
 * Version: 1.0.6
 * Author: Graphikup
 * License: GPLv2 or later
 * License URI: https://www.gnu.org/licenses/gpl-2.0.html
 * Text Domain: acf-gallery-masonry
 */

if (!defined('ABSPATH')) exit;

final class ACF_Gallery_Masonry_Lightbox {

  const VERSION = '1.0.6';
  const HANDLE  = 'acf-gallery-masonry-lightbox';

  public function __construct() {
    add_shortcode('acf_gallery', [$this, 'shortcode']);
    add_action('wp_enqueue_scripts', [$this, 'enqueue_assets']);
  }

  /**
   * Shortcode: [acf_gallery field="galerie" columns="4" size="large" gap="14" class="acf-gallery-masonry"]
   */
  public function shortcode($atts) {
    $atts = shortcode_atts([
      'field'   => '',
      'post_id' => get_the_ID(),
      'size'    => 'large',
      'columns' => 4,
      'gap'     => 14,
      'class'   => 'acf-gallery-masonry',
    ], (array) $atts, 'acf_gallery');

    if (empty($atts['field'])) return '';

    if (!function_exists('get_field')) {
      return '<!-- ACF not active: get_field() missing -->';
    }

    $images = get_field($atts['field'], $atts['post_id']);
    if (empty($images) || !is_array($images)) return '';

    $columns = max(1, (int) $atts['columns']);
    $gap     = max(0, (int) $atts['gap']);

    // Always include base class + allow multiple classes.
    $classes = array_filter(preg_split('/\s+/', (string) $atts['class']));
    $classes[] = 'acf-gallery-masonry';
    $classes = array_unique(array_filter($classes));
    $classes = array_map('sanitize_html_class', $classes);
    $class_attr = trim(implode(' ', $classes));

    $html  = '<div class="'.esc_attr($class_attr).'" data-gap="'.esc_attr((string)$gap).'" data-cols="'.esc_attr((string)$columns).'">';
    $html .= '<div class="grid-sizer"></div>';

    foreach ($images as $img) {
      $id = null;

      // Support ACF return formats: array / ID
      if (is_array($img) && !empty($img['ID'])) {
        $id = (int) $img['ID'];
      } elseif (is_numeric($img)) {
        $id = (int) $img;
      }
      if (!$id) continue;

      $full = wp_get_attachment_image_src($id, 'full');
      if (!$full) continue;

      $full_url = $full[0];
      $caption  = wp_get_attachment_caption($id);

      $thumb = wp_get_attachment_image($id, $atts['size'], false, [
        'loading' => 'lazy',
        'class'   => 'acf-gallery-img',
      ]);

      $html .= '<a class="acf-gallery-item glightbox" href="'.esc_url($full_url).'"';
      if (!empty($caption)) {
        $html .= ' data-title="'.esc_attr(wp_strip_all_tags($caption)).'"';
      }
      $html .= '>';
      $html .= $thumb;
      $html .= '</a>';
    }

    $html .= '</div>';

    // Debug marker for update tests
    $html .= "\n<!-- ACF Gallery Masonry v" . esc_html(self::VERSION) . " -->\n";

    return $html;
  }

  public function enqueue_assets() {
    // ⚠️ Important: do NOT rely on "shortcode executed" here.
    // We enqueue safely on front-end. Very lightweight, and avoids Divi/shortcode timing issues.

    if (is_admin()) return;

    // Masonry (WP core)
    wp_enqueue_script('masonry');

    // imagesLoaded (WP core)
    wp_enqueue_script('imagesloaded');

    // GLightbox
    wp_enqueue_style(
      'glightbox',
      'https://cdn.jsdelivr.net/npm/glightbox/dist/css/glightbox.min.css',
      [],
      null
    );
    wp_enqueue_script(
      'glightbox',
      'https://cdn.jsdelivr.net/npm/glightbox/dist/js/glightbox.min.js',
      [],
      null,
      true
    );

    // CSS inline
    wp_register_style(self::HANDLE, false, [], self::VERSION);
    wp_enqueue_style(self::HANDLE);

    $css = <<<CSS
.acf-gallery-masonry{width:100%;}
.acf-gallery-masonry .grid-sizer,
.acf-gallery-masonry .acf-gallery-item{width:25%;}
@media (max-width:1024px){
  .acf-gallery-masonry .grid-sizer,
  .acf-gallery-masonry .acf-gallery-item{width:33.333%;}
}
@media (max-width:640px){
  .acf-gallery-masonry .grid-sizer,
  .acf-gallery-masonry .acf-gallery-item{width:50%;}
}
.acf-gallery-masonry .acf-gallery-item{display:block;margin-bottom:14px;}
.acf-gallery-masonry .acf-gallery-item img{width:100%;height:auto;display:block;border-radius:6px;}
CSS;
    wp_add_inline_style(self::HANDLE, $css);

    // JS init inline (robust)
    wp_add_inline_script('glightbox', <<<JS
(function(){
  function init(){
    // Lightbox
    if (window.GLightbox) {
      GLightbox({ selector: '.acf-gallery-masonry .glightbox' });
    }

    // Masonry
    document.querySelectorAll('.acf-gallery-masonry').forEach(function(grid){
      var gap = parseInt(grid.dataset.gap || '14', 10);

      // apply gap to bottom spacing (safe)
      grid.querySelectorAll('.acf-gallery-item').forEach(function(item){
        item.style.marginBottom = gap + 'px';
      });

      var initMasonry = function(){
        if (!window.Masonry) return;
        new Masonry(grid, {
          itemSelector: '.acf-gallery-item',
          percentPosition: true,
          columnWidth: '.grid-sizer',
          gutter: gap
        });
      };

      if (window.imagesLoaded) {
        imagesLoaded(grid, initMasonry);
      } else {
        initMasonry();
      }
    });
  }

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', init);
  } else {
    init();
  }
})();
JS, 'after');
  }
}

new ACF_Gallery_Masonry_Lightbox();
